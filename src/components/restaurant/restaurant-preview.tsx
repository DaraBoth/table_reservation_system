  ...
  return (
    <div className="sticky top-12 space-y-6 animate-in fade-in slide-in-from-right-4 duration-1000">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[10px] uppercase font-black tracking-widest text-slate-500">Live Studio Preview</span>
      </div>

      {/* Main Card Preview */}
      <Card className="bg-slate-900/40 backdrop-blur-xl border-slate-800/60 shadow-2xl overflow-hidden group">
        <div className="absolute inset-0 bg-gradient-to-br from-violet-600/10 to-indigo-600/10 opacity-50" />
        <CardHeader className="relative pb-4">
          <div className="flex items-start justify-between">
            <div className="w-16 h-16 rounded-3xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-xl font-black text-white shadow-2xl shadow-violet-500/20 mb-4 border border-violet-400/20 transition-transform duration-500 group-hover:scale-110">
              {initials}
            </div>
            <Badge className={cn(
              "text-[10px] font-black uppercase tracking-tighter px-3",
              isActive ? 'bg-emerald-500 text-black' : 'bg-slate-800 text-slate-500'
            )}>
              {isActive ? 'Live' : 'Draft'}
            </Badge>
          </div>
          <CardTitle className="text-2xl font-black text-white tracking-tight break-words">
            {displayName}
          </CardTitle>
      ...
      <div className="p-6 rounded-3xl bg-violet-600/5 border border-violet-500/10 backdrop-blur-sm">
        ...
        <p className="text-slate-400 text-xs leading-relaxed font-medium">
          Make sure your <span className="text-white">Slug</span> is URL-friendly. Use lowercase letters and hyphens only—this will be the unique link your customers use to book tables.
        </p>
      </div>
    </div>
  )
