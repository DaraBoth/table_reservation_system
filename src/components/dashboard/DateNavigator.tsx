...
export function DateNavigator({ selectedDate, onChange, className }: DateNavigatorProps) {
  ...
  return (
    <div className={cn(
      "w-full flex items-center justify-between bg-slate-900/50 border border-slate-800 p-1.5 rounded-2xl backdrop-blur-md shadow-xl",
      className
    )}>
      {/* ⬅️ Previous Day */}
      <button
        type="button"
        onClick={() => shiftDate(-1)}
        className="w-10 h-10 rounded-xl bg-slate-950 border border-slate-800 hover:border-violet-500/50 text-slate-400 hover:text-white transition-all active:scale-90 flex items-center justify-center p-0"
      >
      ...
      <div className="flex flex-col items-center px-4 min-w-[140px]">
        <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-0.5">
          <CalendarDays className="w-3 h-3" />
          {isSelectedToday ? 'Showing Today' : format(date, 'EEEE')}
        </div>
        <p className="text-sm font-black text-white italic tracking-tight uppercase leading-none">
