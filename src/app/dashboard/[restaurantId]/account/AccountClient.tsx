...
export function AccountClient({ user, membership, profile }: AccountClientProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 md:py-12 min-h-screen">
      <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-8 md:gap-16">
        ...
        <aside className="space-y-8">
          <div className="flex flex-col gap-1 sticky top-12">
            <h1 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 mb-4 px-2">Settings</h1>
            {menu.map((item) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? "bg-violet-600/10 text-violet-400" : "text-slate-500 hover:text-slate-300 hover:bg-slate-900/50"
                  )}
                >
                  {active && (
                    <motion.div layoutId="nav" className="absolute inset-0 bg-violet-600/5 rounded-2xl border border-violet-500/20" />
                  )}
                  <Icon className={cn("w-4.5 h-4.5 relative z-10", active ? "text-violet-400" : "text-slate-600 group-hover:text-slate-400")} />
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === 'profile' && (
                <div className="space-y-10">
                  <header className="mb-8">
                    <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter">Profile</h2>
                    <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mt-1 opacity-70 italic">Your info.</p>
                  </header>
                  ...
                  <div className="space-y-12">
                    <div className="flex items-center gap-6 pb-6 border-b border-slate-900">
                      ...
                      <div>
                        <p className="text-sm font-black text-white italic uppercase tracking-tight">{profile?.full_name}</p>
                        <p className="text-[10px] font-black uppercase tracking-widest text-violet-500 mt-0.5">{membership?.role}</p>
                        <p className="text-[10px] text-slate-600 font-bold tracking-tight">{user?.email}</p>
                      </div>
                    </div>
                    ...
                    <form action={profileAction} className="space-y-8">
                        <div className="space-y-3 px-1">
                          ...
                          <Input
                            ...
                            className="h-14 bg-slate-950 border-slate-800 rounded-2xl text-lg font-bold text-white px-5 focus-visible:border-violet-500 focus-visible:ring-violet-500/20 transition-all shadow-sm placeholder:text-slate-800"
              ...
              {activeTab === 'security' && (
                <div className="space-y-10">
                  <header>
                    <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter">Security</h2>
                    <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mt-1 opacity-70 italic">Secure your account.</p>
                  </header>
                  ...
                  <div className="space-y-12">
                     <form action={passwordAction} className="space-y-8">
                        ...
                        <div className="space-y-3 px-1">
                          <Label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">New Pass</Label>
                          <Input
                            name="newPassword"
                            type="password"
                            className="h-14 bg-slate-950 border-slate-800 rounded-2xl text-lg font-bold text-white px-5 focus-visible:border-violet-500 focus-visible:ring-violet-500/20 transition-all shadow-sm placeholder:text-slate-800"
                        ...
                        <div className="space-y-3 px-1">
                          <Label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Confirm</Label>
                          <Input
              ...
              {activeTab === 'business' && (
                <div className="space-y-10">
                  <header>
                    <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter">Business</h2>
                    <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mt-1 opacity-70 italic">Business info.</p>
                  </header>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                        <div className="space-y-3 col-span-full px-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <Label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Business Name</Label>
                          </div>
                          <Input
                            name="name"
                            defaultValue={membership?.restaurants?.name}
                            className="h-14 bg-slate-950 border-slate-800 rounded-2xl text-lg font-bold text-white px-5 focus-visible:border-violet-500 focus-visible:ring-violet-500/20 transition-all shadow-sm"
                          />
                        </div>
                        ...
                        <div className="space-y-3 col-span-full px-1">
                           <div className="flex items-center gap-2 mb-1">
                            ...
                            <Label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Logo URL</Label>
                          </div>
                          <Input
                            name="logoUrl"
      ...
      {/* Native App Footer */}
      <footer className="mt-24 pt-12 border-t border-slate-900/50 flex flex-col items-center gap-2 opacity-30">
        <Globe className="w-4 h-4 text-slate-600" />
        <p className="text-[9px] text-slate-600 font-black uppercase tracking-[0.4em]">TableBook OS · iPad Pro Edition</p>
      </footer>
    </div>
  )
}
