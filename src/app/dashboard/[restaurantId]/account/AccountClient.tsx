...
export function AccountClient({ user, membership, profile }: AccountClientProps) {
  ...
  const storeName = membership?.restaurants?.name || t('account.store')
  ...
  const checkAvailability = async () => {
    ...
    if (username.length < 2) {
      setUsernameError(t('account.min2'))
      setIsAvailable(false)
      return
    }
    if (!/^[a-z0-9_.]+$/.test(username)) {
      setUsernameError(t('account.lowercaseOnly'))
      setIsAvailable(false)
      return
    }
    ...
    try {
      ...
      if (!res.available) setUsernameError(t('account.alreadyTaken'))
    } catch (e) {
  ...
  const menu = [
    { id: 'profile', icon: UserCircle, label: t('account.myInfo') },
    { id: 'security', icon: ShieldCheck, label: t('account.password') },
    ...(isAdmin ? [{ id: 'business', icon: StoreIcon, label: t('account.store') }] : []),
  ]
  ...
  return (
    <div className="mx-auto min-h-screen max-w-6xl px-4 py-6 md:px-6 md:py-8 xl:px-8 xl:py-12">
      <div className="grid grid-cols-1 gap-8 xl:grid-cols-[240px_minmax(0,1fr)] xl:gap-14">
        ...
        <aside className="xl:hidden">
          <div className="rounded-[28px] border border-border/50 bg-card/35 p-3 backdrop-blur-sm">
            <div className="mb-3 px-2">
              <h1 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">{t('account.settings')}</h1>
            </div>
                ...
                {menu.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(getTabButtonClassName(active), "min-w-40 justify-center")}
                    >
                      {active && (
        ...
        <aside className="hidden xl:block xl:space-y-8">
          <div className="sticky top-12 flex flex-col gap-1">
            <h1 className="mb-4 px-2 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">{t('account.settings')}</h1>
            {menu.map((item) => {
              const Icon = item.icon
              const active = activeTab === item.id
              return (
            ...
            >
              {activeTab === 'profile' && (
                <div className="space-y-8 md:space-y-10">
                  <header className="mb-6 md:mb-8">
                    <h2 className="text-2xl font-black text-foreground italic uppercase tracking-tighter">{t('account.myInfo')}</h2>
                    <p className="mt-1 text-xs font-bold uppercase tracking-wider text-muted-foreground opacity-70 italic">{t('account.updateNamePhoto')}</p>
                  </header>
                  ...
                  <div className="space-y-8 md:space-y-10">
                    <div className="grid gap-6 border-b border-border pb-6 md:grid-cols-[auto_minmax(0,1fr)] md:items-center md:gap-8">
                      <AvatarUpload 
                        ...
                        userName={profile?.full_name || t('account.myInfo')}
                        onUpload={async (blob) => {
                          ...
                          toast.success(t('media.photoSaved'))
                        }}
                      />
                      <div className="min-w-0 text-center md:text-left">
                        <p className="text-xl font-black text-foreground italic uppercase tracking-tight">{profile?.full_name}</p>
                        <p className="text-xs font-black uppercase tracking-widest text-violet-500 mt-0.5">{t(`roles.${membership?.role}`, { defaultValue: membership?.role || '' })}</p>
                    ...
                    <form action={profileAction} className="space-y-6 md:space-y-8 md:max-w-2xl">
                        <div className="grid gap-6 md:grid-cols-2">
                          <div className="space-y-3 px-1">
                            <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{t('account.name')}</Label>
                            <Input
                              name="fullName"
                              value={name}
                          ...
                          <div className="space-y-3 px-1">
                            <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{t('account.username')}</Label>
                            <div className="relative group">
                              ...
                              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
                                ...
                                >
                                  {isChecking ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : t('account.check')}
                                </Button>
                              </div>
                            </div>
                            {usernameError && (
                            ...
                            <p className="px-1 text-[10px] font-bold text-muted-foreground/40 mt-1 flex items-center gap-1.5">
                              ...
                              {t('account.loginIdentifier')}
                            </p>
                          </div>
                        </div>

                      {profileState?.success && (
                        <p className="text-xs text-emerald-400 font-black uppercase tracking-widest flex items-center gap-2 animate-in fade-in slide-in-from-left-2"><Check className="w-3.5 h-3.5" /> {t('account.profileUpdated')}</p>
                      )}
                      {profileState?.error && (
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        {username !== initialUsername && isAvailable === null && (
                           <p className="text-[9px] font-black uppercase tracking-widest text-amber-500 animate-pulse">{t('account.checkBeforeSave')}</p>
                        )}
                      </div>
                    </form>
                  </div>
              ...
              {activeTab === 'security' && (
                <div className="space-y-8 md:space-y-10 md:max-w-2xl">
                  <header>
                    <h2 className="text-2xl font-black text-foreground italic uppercase tracking-tighter">{t('account.password')}</h2>
                    <p className="mt-1 text-xs font-bold uppercase tracking-wider text-muted-foreground opacity-70 italic">{t('account.updatePassword')}</p>
                  </header>
                  ...
                  <div className="space-y-12">
                     <form action={passwordAction} className="space-y-8">
                        ...
                        <div className="space-y-3 px-1">
                          <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{t('account.newPassword')}</Label>
                          <Input
                            name="newPassword"
                            type="password"
                            className="h-14 bg-background border-border rounded-2xl text-lg font-bold text-foreground px-5 focus-visible:border-violet-500 focus-visible:ring-violet-500/20 transition-all shadow-sm placeholder:text-muted-foreground/20"
                        ...
                        <div className="space-y-3 px-1">
                          <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{t('account.confirmPassword')}</Label>
                          <Input
                        ...
                        {passwordState?.success && (
                          <p className="text-xs text-emerald-400 font-black uppercase tracking-widest flex items-center gap-2 animate-in fade-in slide-in-from-left-2"><Check className="w-3.5 h-3.5" /> {t('account.passwordSaved')}</p>
                        )}
                        {passwordState?.error && (
                        ...
                        <Button type="submit" disabled={passwordPending} className="h-10 px-8 bg-violet-600 hover:bg-violet-500 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-xl shadow-violet-500/10 active:scale-95 transition-all">
                          {passwordPending ? t('common.saving') : t('common.save')}
                        </Button>
                     </form>
                  </div>
                </div>
              ...
              {activeTab === 'business' && (
                <div className="space-y-8 md:space-y-10">
                  <header>
                    <h2 className="text-2xl font-black text-foreground italic uppercase tracking-tighter">{t('account.store')}</h2>
                    <p className="mt-1 text-xs font-bold uppercase tracking-wider text-muted-foreground opacity-70 italic">{t('account.updateStore')}</p>
                  </header>
                  ...
                  <div className="space-y-8 md:space-y-10">
                    <form action={businessAction} className="space-y-10">
                      ...
                      <div className="grid items-center gap-8 border-b border-border/50 pb-8 md:grid-cols-[auto_minmax(0,1fr)] md:gap-10 md:pb-10">
                        <LogoUpload 
                          ...
                          onUpload={async (blob) => {
                            ...
                            router.refresh()
                            toast.success(t('media.logoSaved'))
                          }}
                        />
                        <div className="min-w-0 space-y-1 text-center md:text-left">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-violet-500/60">{t('account.logoAndLink')}</p>
                          <p className="text-xs text-muted-foreground/60 font-bold max-w-md leading-relaxed mt-2">
                            {t('account.logoDescription')}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-x-10 gap-y-8 lg:grid-cols-2">
                        <div className="space-y-3 col-span-full px-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{t('account.storeName')}</Label>
                          </div>
                          <Input
                            name="name"
                        ...
                        <div className="space-y-3 col-span-full px-1">
                          <div className="mb-1 flex items-center gap-2">
                            ...
                            <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">{t('account.shortLink')}</Label>
                          </div>
                          <Input
                            name="slug"
                            defaultValue={storeSlug}
                            placeholder={t('account.myStoreSlug')}
                            className="h-14 bg-background border-border rounded-2xl text-lg font-bold text-foreground px-5 focus-visible:border-violet-500 focus-visible:ring-violet-500/20 transition-all shadow-sm"
                          />
                          <p className="px-1 text-[11px] font-bold text-muted-foreground/60">
                            {t('account.shortLinkHelp')}
                          </p>
                        </div>
                        ...
                        <div className="space-y-3 px-1">
                           <div className="flex items-center gap-2 mb-1">
                            ...
                            <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{t('account.email')}</Label>
                          </div>
                          <Input
                            name="contactEmail"
                        ...
                        <div className="space-y-3 px-1">
                           <div className="flex items-center gap-2 mb-1">
                            ...
                            <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{t('account.phone')}</Label>
                          </div>
                          <Input
                            name="contactPhone"
                        ...
                        <div className="space-y-3 col-span-full px-1">
                           <div className="flex items-center gap-2 mb-1">
                            ...
                            <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">{t('account.address')}</Label>
                          </div>
                          <Input
                            name="address"
                      ...
                      {businessState?.success && (
                        <p className="text-xs text-emerald-400 font-black uppercase tracking-widest flex items-center gap-2 animate-in fade-in slide-in-from-left-2"><Check className="w-3.5 h-3.5" /> {t('account.saved')}</p>
                      )}
                      ...
                      <Button type="submit" disabled={businessPending} className="h-10 px-8 bg-violet-600 hover:bg-violet-500 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-xl shadow-violet-500/10 active:scale-95 transition-all">
                        {businessPending ? t('common.saving') : t('common.save')}
                      </Button>
                    </form>
                  </div>
                </div>
      ...
      <footer className="mt-16 flex flex-col items-center gap-2 border-t border-border/50 pt-10 opacity-30 md:mt-20 md:pt-12">
        ...
        <p className="text-[9px] text-muted-foreground/60 font-black uppercase tracking-[0.4em]">{t('account.madeForIpad')}</p>
      </footer>
    </div>
  )
}
