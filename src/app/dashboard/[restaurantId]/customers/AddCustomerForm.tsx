    ...
    <Sheet open={open} onOpenChange={setOpen}>
        ...
        render={
          (trigger as React.ReactElement) || (
            <Button className="fixed bottom-24 right-5 w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600 border-0 shadow-2xl shadow-violet-500/40 z-30 group active:scale-90 transition-all duration-300">
              ...
              <span className="sr-only">{t('dashboard.addCustomer', { defaultValue: 'Add Customer' })}</span>
            </Button>
          )
        }
      />
      <SheetContent side={isMobile ? 'bottom' : 'right'} className={`bg-card border-border text-foreground p-6 ${isMobile ? 'rounded-t-3xl' : ''}`}>
        <SheetHeader className="p-0 mb-4">
          <SheetTitle className="text-foreground text-lg font-black italic tracking-tight">{t('dashboard.createUserProfile', { defaultValue: 'Create User Profile' })}</SheetTitle>
          <p className="text-xs text-muted-foreground">{t('dashboard.saveContactDetailsForFuture', { defaultValue: 'Quickly save contact details for future seatings' })}</p>
        </SheetHeader>
        <form action={action} className="space-y-4 mt-2">
          ...
          <div>
            <label className="block text-xs font-bold text-muted-foreground mb-2 uppercase tracking-widest">
              {t('account.phone', { defaultValue: 'Phone' })}
            </label>
            <input
              name="phone"
              type="tel"
          ...
          {(state as any)?.success && (
            <p className="text-xs text-emerald-400 font-bold bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-3 py-2 flex items-center gap-1.5">
              <CircleCheck className="w-3 h-3 flex-shrink-0" /> {t('dashboard.customerSaved', { defaultValue: 'Customer saved!' })}
            </p>
          )}
          ...
          >
            {pending ? t('common.saving', { defaultValue: 'Saving...' }) : t('dashboard.saveCustomer', { defaultValue: 'Save Customer' })}
          </button>
        </form>
      </SheetContent>
    </Sheet>
