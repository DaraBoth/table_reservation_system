  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto pb-20">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: `Total ${terms.bookings}`, count: totalToday, icon: CalendarDays, color: 'violet', href: '/dashboard/reservations' },
          { label: 'Waiting', count: pendingCount, icon: Clock, color: 'amber', href: '/dashboard/reservations' },
          { label: terms.units, count: totalTables, icon: UnitIcon, color: 'emerald', href: '/dashboard/tables' }
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/dashboard/reservations" className="text-[10px] text-violet-400 font-black uppercase tracking-widest hover:text-violet-300 transition-colors">
            See all →
          </Link>
        </div>
