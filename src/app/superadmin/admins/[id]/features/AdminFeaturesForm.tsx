  ...
  const AVAILABLE_FEATURES = [
    {
      ...
      label: t('superadmin.featureMultiBrandLabel', { defaultValue: 'Multi-Brand Expansion' }),
      description: t('superadmin.featureMultiBrandDesc', { defaultValue: 'Allows this admin to create and manage their own portfolio of properties independently.' }),
      icon: PlusCircle
    },
    {
      id: 'analytics_pro',
      label: t('superadmin.featureAnalyticsProLabel', { defaultValue: 'Advanced Intelligence' }),
      description: t('superadmin.featureAnalyticsProDesc', { defaultValue: 'Access to cross-property analytics and enterprise-grade reporting modules.' }),
      icon: LayoutGrid
    },
  ]
    ...
    <div className="space-y-4 pb-10">
      ...
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-black text-foreground uppercase tracking-tight">{t('superadmin.entitlementsTitle', { defaultValue: 'Entitlements' })}</h1>
            <p className="text-[10px] text-muted-foreground font-medium">
              {t('superadmin.identityLabel', { defaultValue: 'Identity:' })} <span className="text-foreground font-black">{name}</span> {t('superadmin.atLabel', { defaultValue: 'at' })} <span className="text-foreground font-bold">{property}</span>
            </p>
          </div>
        </div>
        ...
        >
          {isPending ? (
            ...
            <>
              ...
              {t('superadmin.commitUpdates', { defaultValue: 'Commit Updates' })}
            </>
          )}
        </Button>
      </div>
        ...
        <div className="p-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-0.5">
              <h2 className="text-sm font-black text-foreground uppercase tracking-tight">{t('superadmin.specialAdminStatus', { defaultValue: 'Special Admin Status' })}</h2>
              <p className="text-muted-foreground/60 text-[9px] font-bold uppercase tracking-widest italic">{isSpecial ? t('superadmin.privilegedAccessEnabled', { defaultValue: 'Privileged platform access enabled' }) : t('superadmin.standardAdminConstraints', { defaultValue: 'Standard administrator constraints applied' })}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 bg-black/40 px-3 py-2 rounded-xl border border-white/5 backdrop-blur-sm">
            ...
            <Label htmlFor="special-toggle" className={cn("text-[9px] font-black uppercase tracking-widest", isSpecial ? "text-violet-400" : "text-muted-foreground/60")}>
              {isSpecial ? t('superadmin.statusActive', { defaultValue: 'Active' }) : t('superadmin.statusOff', { defaultValue: 'Off' })}
            </Label>
          </div>
        </div>
      </div>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2 px-1">
           ...
           <h3 className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-widest">{t('superadmin.platformModules', { defaultValue: 'Platform Modules' })}</h3>
        </div>
              ...
              <div key={feat.id} className="space-y-2">
                ...
                {isSelected && feat.id === 'create_restaurant' && (
                  <div className="ml-1 px-3 py-2 bg-black/30 border border-border/50 rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                      <Label className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">{t('superadmin.brandQuota', { defaultValue: 'Brand Quota' })}</Label>
                      <span className="text-[11px] font-black text-violet-400 italic">{t('superadmin.unitsCount', { count: config.max_brands || 1, defaultValue: '{{count}} UNIT(S)' })}</span>
                    </div>
                    <input
                      type="range"
                      min="1"
                    ...
                    />
                    <p className="text-[8px] text-muted-foreground/60 font-bold uppercase">{t('superadmin.brandQuotaDescription', { defaultValue: 'Limits the total number of brands this admin can establish.' })}</p>
                  </div>
                )}
              </div>
            )
      ...
      {/* Footer Info Only */}
      <div className="pt-4">
        <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">{t('superadmin.uidRef', { id: membershipId.slice(0, 8), defaultValue: 'UID_REF: {{id}}' })}</p>
      </div>
    </div>
  )
}
