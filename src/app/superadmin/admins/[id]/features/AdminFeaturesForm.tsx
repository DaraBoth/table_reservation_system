    ...
    <div className="space-y-4 pb-10">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div className="flex items-center gap-4">
          <Link 
            ...
            className={cn(
              ...
              "w-8 h-8 p-0 rounded-lg text-slate-500 hover:text-white hover:bg-white/5 border border-slate-800"
            )}
          >
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="flex flex-col">
            <h1 className="text-xl font-black text-white uppercase tracking-tight">Entitlements</h1>
            <p className="text-[10px] text-slate-500 font-medium">
              Identity: <span className="text-white font-black">{name}</span> at <span className="text-white font-bold">{property}</span>
            </p>
          </div>
        </div>
        ...
        <Button 
          onClick={handleSave}
          disabled={isPending}
          className="h-9 px-5 bg-violet-600 hover:bg-violet-500 text-white rounded-lg font-black text-[10px] uppercase tracking-[0.2em] shadow-md shadow-violet-500/5 gap-2 group"
        >
          {isPending ? (
        ...
        <div className="p-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className={cn(
              ...
              isSpecial ? "bg-violet-600/20 text-violet-400" : "bg-slate-800 text-slate-600"
            )}>
              <ShieldCheck className="w-5 h-5" />
            </div>
            <div className="space-y-0.5">
              <h2 className="text-sm font-black text-white uppercase tracking-tight">Special Admin Status</h2>
              <p className="text-slate-600 text-[9px] font-bold uppercase tracking-widest italic">{isSpecial ? 'Privileged platform access enabled' : 'Standard administrator constraints applied'}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 bg-black/40 px-3 py-2 rounded-xl border border-white/5 backdrop-blur-sm">
            ...
            />
            <Label htmlFor="special-toggle" className={cn("text-[9px] font-black uppercase tracking-widest", isSpecial ? "text-violet-400" : "text-slate-600")}>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2 px-1">
           ...
           <h3 className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Platform Modules</h3>
        </div>
              ...
              <div key={feat.id} className="space-y-2">
                <div 
                  ...
                  className={cn(
                    ...
                    isSelected 
                      ? "bg-slate-800 text-violet-400 border-violet-500/30 shadow-md" 
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      ...
                      isSelected ? "bg-violet-500 text-white" : "bg-slate-800 text-slate-500"
                    )}>
                      <Icon className="w-4 h-4" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <span className="font-black text-white group-hover:text-violet-300 transition-colors uppercase tracking-tight text-[11px] italic truncate mr-2">{feat.label}</span>
                        <Checkbox 
                          checked={isSelected}
                          className="w-3.5 h-3.5 rounded-sm border-slate-700 data-[state=checked]:bg-violet-500"
                        />
                      </div>
                    </div>
                  </div>
                ...
                {isSelected && feat.id === 'create_restaurant' && (
                  <div className="ml-1 px-3 py-2 bg-black/30 border border-slate-800/50 rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                      <Label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Brand Quota</Label>
                    ...
                    />
                    <p className="text-[8px] text-slate-600 font-bold uppercase">Limits the total number of brands this admin can establish.</p>
                  </div>
                )}
              </div>
            )
      ...
      {/* Footer Info Only */}
      <div className="pt-4">
        <p className="text-[8px] font-black text-slate-800 uppercase tracking-widest">UID_REF: {membershipId.slice(0, 8)}</p>
      </div>
    </div>
  )
}
