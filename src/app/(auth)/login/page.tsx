...
function LoginErrorBanner() {
  ...
  const messages: Record<string, string> = {
    ...
    restaurant_suspended: t('auth.restaurantSuspended'),
    subscription_expired: t('auth.subscriptionExpired'),
  }
...
function LoginForm() {
  ...
  const handleFastLogin = (account: any) => {
    ...
    startTransition(async () => {
      ...
      const res = await login(null, formData)
      if (res?.error) {
        setErrorMsg(t('auth.savedPasswordChanged', { error: res.error }))
      } else if (res?.success && res.url) {
        router.push(res.url)
      }
    })
        ...
        <div className="w-full max-w-sm">
          ...
          {isClient && savedAccounts.length > 0 && (
            <div className="mb-8 w-full">
              <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mb-4">{t('auth.savedAccounts')}</p>
          ...
          <div className="mb-8 text-center md:text-left">
            <h2 className="text-2xl font-bold text-foreground mb-1 tracking-tight">{t('auth.loginTitle')}</h2>
